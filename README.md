...
* Known issue in data: missing `press` data between the timestamps 73m 47s to 89m 40s, as indicated by data in the `press_x` and `press_y` columns. No plan to fix existing dataset in this repo, however future workshop~jupyterlite set-ups will have the resolved this missing data (but other data will not be identical/will have been updated in line with the design and intention of that iteration).
